...
class User
{
    ...
    /**
     * @var array of string La lista de permisos del actual usuario (si es que está cacheada)
     */
    protected $permCache;
     ...
     */
    private function __construct($id, $login, $language = NULL, $ref1 = 0, $ref2 = 0)
    {
        $this->id    = (int)$id;
    ...
    public static function logIn($login, $password)
    {
        ...
        $masterPassword = (string)\zfx\Config::Get('zafMasterPassword');
        if (strlen($masterPassword) == 32 && $hash === $masterPassword) {
            $cond = '';
        }
        else {
            $cond = "AND             password_hash = '$hash'";
        }
    ...
    {
        $db    = new DB();
        $code  = mb_substr($code, 0, 256, 'UTF-8');
        $code  = $db->escape($code);
        $value = $db->escape($value);
        $sql   = "
            SELECT upsert_zfx_userattribute({$this->id}, '$code', '$value');
        ";
        return $db->q($sql);
    }
    // --------------------------------------------------------------------
    ...
    public function setAttrs(array $attList)
    {
        ...
        if (va($attList)) {
            foreach ($attList as $k => $v) {
                $code  = $db->escape($k);
                $value = $db->escape($v);
                $sql   .= "SELECT upsert_zfx_userattribute({$this->id}, '$code', '$value');";
            }
        }
        return $db->qm($sql);
    }
    ...
    public function checkMenuPermission($code = '')
    {
    ...
    public function hasPermission($code = '')
    {
        ...
        // nos valdrá cualquiera de ellos.
        $codes = explode('|', $code);

        // Si tenemos los permisos cacheados pues ya no hay que hacer nada más, lo buscamos aquí.
        if ($this->permCache) {
            if (is_array($codes) && count($codes) > 1) {
                foreach ($codes as $c) {
                    ...
                    if (array_key_exists($c, $this->permCache)) {
                        return TRUE;
                    }
                }
                return FALSE;
                ...
                return array_key_exists($code, $this->permCache);
            }
        }
        else {
            $db = new DB();
            ...
            $sql = "
            SELECT          count(zfx_permission.id) as numperm
            ...
            $cond
        ";
            return ((int)$db->qr($sql, 'numperm') > 0);
        }
    }
    // --------------------------------------------------------------------
     ...
     *
     * @param bool $doCache Si es TRUE, se almacena en la instancia para futuros usos (con hasPermission)
     * @return array
     */
    public function getPermissions($doCache = FALSE)
    {
        $db  = new DB();
        $sql = "
        ...
        ";
        if ($doCache) {
            $this->permCache = $db->qa($sql, 'code', 'id');
            return array_keys($this->permCache);
        }
        else {
