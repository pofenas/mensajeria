...
class FieldDate extends Field
{
    ...
    public static function cast($data, $required = FALSE)
    {
        if (is_a($data, '\DateTime')) {
            $value = $data->format(Config::get('dbDateFormat'));
        }
        else {
            if (!DateTime::createFromFormat(Config::get('dbDateFormat'), $data)) {
                $value = NULL;
            }
            else {
                $value = $data;
    ...
    {
        if ($required) {
            return $loc->interpretDate($data, TRUE);
        }
        else {
    ...
    public function setDefaultValue($value)
    {
        if (is_a($value, '\DateTime')) {
            ...
            $this->defaultValue = DateTime::createFromFormat(Config::get('dbDateFormat'), $value);
        }
    }
    ...
    {
        $d = date_create_from_format(Config::get('dbDateFormat'), $data);
    ...
    {
        return $loc->getDate(self::value($data));
    }
