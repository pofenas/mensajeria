...
class FieldDateTime extends Field
{
    ...
    public static function cast($data, $required = FALSE)
    {
        if (is_a($data, '\DateTime')) {
            $value = $data->format(Config::get('dbDateTimeFormat'));
        }
        else {
            if (!DateTime::createFromFormat(Config::get('dbDateTimeFormat'), $data)) {
                $value = NULL;
            }
            else {
                $value = $data;
    ...
    {
        if ($required) {
            return $loc->interpretDateTime($data, TRUE);
        }
        else {
    ...
    public function setDefaultValue($value)
    {
        if (is_a($value, '\DateTime')) {
            ...
            $this->defaultValue = DateTime::createFromFormat(Config::get('dbDateTimeFormat'), $value);
        }
    }
    ...
    {
        $d = date_create_from_format(Config::get('dbDateTimeFormat'), $data);
    ...
    {
        return $loc->getDateTime(self::value($data));
    }
